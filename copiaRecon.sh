...
if [ -e $caminho"/scanner/"$1"/resultados/subs.txt" ];then
	...
	#Confirmando o caminho de diretório
	data=$(date +"%H:%M:%S");
    echo -e "\n\033[1;34;40m"$data"\033[m Resultados: \033[32;1m"$pasta"/scanner/"$1"/resultados/\033[m";
	...
	cat $findomain sublisterLimpo.txt | sort -u > subs.txt;
	...
	function recon {
		...
		data=$(date +"%H:%M:%S");
        echo -e "\033[1;34;40m"$data"\033[m \033[32;1m <<Recon de parâmetros utilizando GAU ou GAU + HAKRAWLER/>>>\033[m";


		data=$(date +"%H:%M:%S");
        echo -e "\033[1;34;40m"$data"\033[m \033[32;1m Mesclando URLS GAU + URLS HAKRAWLER...\033[m";
		data=$(date +"%H:%M:%S");
    	echo -e "\033[1;34;40m"$data"\033[m Arquivo a ser gerado: gauAndHakraw.txt";
		sleep 2;
		caminho=$(pwd)

		cat $caminho"/scanner/"$dominio"/resultados/"gau.txt $caminho"/scanner/"$dominio"/resultados/"hakraw.txt| sort -u > $caminho"/scanner/"$dominio"/resultados/"gauAndHakraw.txt;

		sleep 2;

		#Definindo variável para GAU e Hakrawler
		gauAndHakraw="gauAndHakraw.txt"

		#_variaveis

        caminho=$(pwd);

        if [ -e $caminho"/scanner/"$dominio"/resultados/"$gauAndHakraw ];then
			...
			cat $pasta"/scanner/"$dominio"/resultados/"$gauAndHakraw | grep "=" | sort -u > $pasta"/scanner/"$dominio"/resultados/"TodosParamsPreenchidos.txt;
            ...
            cat $pasta"/scanner/"$dominio"/resultados/"$gauAndHakraw | grep "=" | sed 's/=.*/=/' > $pasta"/scanner/"$dominio"/resultados/"paramRepetido.txt;
            ...
            if [ -e $caminho"/scanner/"$dominio"/resultados/"$gauAndHakraw ];then
            	cat $pasta"/scanner/"$dominio"/resultados/"$gauAndHakraw | grep "=" | httpx -silent -status-code -mc 200,201,202,203,204,205,206,300,301,302,303,304,307,308,401,403,405,409,411,451,500,501,505,506,507,508,510,511 -o $pasta"/scanner/"$dominio"/resultados/"gauHakrawParamsDuzentosRedirectProibidosServer.txt;
            	...
            	data=$(date +"%H:%M:%S");
    			echo -e "\033[1;34;40m"$data"\033[m Nao encontrei o $gauAndHakraw para filtrar os parâmetros...";
            fi
    ...
    function amasss {
	    data=$(date +"%H:%M:%S");
	    echo -e "\033[1;34;40m"$data"\033[m \033[05;33m<<AMASS EM FUNCIONAMENTO/>\033[00;37m";
	    ...
	    amass enum -d $dom > resultaAmass.txt;
	    cat subs.txt resultaAmass.txt | sort -u > subs_full.txt;
	    rm subs.txt;
    ...
    case $resposta in
    	...
    	*)
        	data=$(date +"%H:%M:%S");
        	echo -e "\033[1;34;40m"$data"\033[m \033[32;1mPor favor insira um valor valido\033[m"
        	exit;;
    	esac
	...
	if [ -e $caminho"/scanner/"$1"/resultados/"$resolv ];then
		...
		python3 scan_hiddens.py;

    	for url in $(cat $pasta"/scanner/"$1"/resultados/"subsResolvidos.txt);do python3 scan_upload.py $url;done >> $pasta"/scanner/"$1"/resultados/"inputsUploads.txt;
